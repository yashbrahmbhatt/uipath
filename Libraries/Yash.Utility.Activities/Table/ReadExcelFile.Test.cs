            ...
            if (testCase.ExpectedOutput != null)
            {
                ...
                else
                {
                    for (int i = 0; i < result.Tables.Count; i++)
                    {
                        ...
                        var expecteDataTableable = testCase.ExpectedOutput.Tables[i];

                        // Check if the number of columns matches.
                        if (actualTable.Columns.Count != expecteDataTableable.Columns.Count)
                        {
                            testing.VerifyExpression(false, $"-\tTest {testCase.Id} failed. Table {i} column count mismatch. Expected {expecteDataTableable.Columns.Count}, but got {actualTable.Columns.Count}");
                            break;
                        }
                        ...
                        for (int j = 0; j < actualTable.Rows.Count; j++)
                        {
                            ...
                            var expectedRow = expecteDataTableable.Rows[j].ItemArray;
