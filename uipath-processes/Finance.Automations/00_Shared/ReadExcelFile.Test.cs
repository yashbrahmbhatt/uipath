...
namespace Finance.Automations._00_Shared
{
    ...
    {
        private readonly string _testDataPath = Path.Combine(Directory.GetCurrentDirectory(), "00_Shared", "TestData", "ReadExcelFile");
        ...
        public void Execute()
        {
            Log($"Running all tests for {nameof(ReadExcelFile)}");
            ...
            Log($"Finished all tests for {nameof(ReadExcelFile)}: PASS: {results.Values.Count(v => v)} FAIL: {results.Values.Count(v => !v)}");
        }
        ...
        public void RunTestCase(ReadExcelFile_TestCase testCase)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Log($"Test {testCase.Id} failed as expected with exception: {ex.Message}");
                    return;
                }
                throw new Exception($"Test {testCase.Id} failed unexpectedly: {ex.Message}");
            }
            ...
            if (testCase.ExpectedOutput != null)
            {
                ...
                {
                    throw new Exception($"Test {testCase.Id} failed. Expected {testCase.ExpectedOutput.Tables.Count} tables, but got {result.Tables.Count}");
                }
                ...
                for (int i = 0; i < result.Tables.Count; i++)
                {
                    ...
                    {
                        throw new Exception($"Test {testCase.Id} failed. Table {i} column count mismatch. Expected {expectedTable.Columns.Count}, but got {actualTable.Columns.Count}");
                    }
                    ...
                    for (int j = 0; j < actualTable.Rows.Count; j++)
                    {
                        ...
                        for (int col = 0; col < actualRow.Length; col++)
                        {
                            ...
                            {
                                throw new Exception($"Test {testCase.Id} failed. Table {i} Row {j}, Column {col} mismatch. " +
                                    $"Expected: {JsonConvert.SerializeObject(expectedRow[col])}, Actual: {JsonConvert.SerializeObject(actualRow[col])}");
                            }
                        }
                    }
    ...
    public class ReadExcelFile_TestCases
    {
        ...
        public static ReadExcelFile_TestCase EmptyExcelFile { get; } = new()
        {
            ...
            ExpectedOutput = new() { Tables = new List<DataTable>() }
        };
    }
